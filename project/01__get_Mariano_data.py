...
for id in range(n):
    ...
    searchedItem = geocode("Mariano", yourZip['extent'], max_locations=1000)
    print("ID - " + str(id), end=" : ")
    ...
    searchedItem = geocode("Mariano's Fresh Market", yourZip['extent'], max_locations=1000)
    print("ID - " + str(id), end=" : ")
    print("ZIPCODE - " + str(uniqueZip[id]), end=" : ")
    print("NUM - " + str(len(searchedItem)))
    for item2 in range(len(searchedItem)):
...
listedList.drop_duplicates(subset=['ADDRESS','POSTAL'],inplace=True)
print(listedList.shape)
