...
import program_01_get_ARCGIS as ARCGIS
import program_02_get_grocery as grocery
import program_03_zillow as zillow 
import program_04_crime as crime 
import program_05_googleSearch as search
...
if __name__ == '__main__':
    ...
    prop_df = pd.merge(prop_df,eco_df,left_on="LOCATION",right_on="COMMUNITY AREA NAME",how="left")
    ...
    # Read in ZipCode data (only view Cook County)
    zips = pd.read_csv("data_sets/illinois_ZIP.csv")
    zips = zips[zips['County'] == "Cook"]
    uniqueZip = zips['Zip Code'].unique()

    # Loop over list of keywords and create/read-in datasets  
    keywordList = [["ALDI"],["Jewel Osco","Jewel-Osco"],["Mariano","Marianos Fresh Market"],["Trader Joes"],["Whole Foods"]]
    groceryList = [] 
    for keyWord in keywordList: 
        print("Gathering Search Result: " + keyWord[0] + "...")
        csv_zipcode = "data_sets/grocery_" + keyWord[0] + ".csv"
        try:
            info = pd.read_csv(csv_zipcode)
        except: 
            info = ARCGIS.get_data_from_arcGIS_api(uniqueZip,keyWord)
            info['NAME'] = keyWord[0]
            info.to_csv(csv_zipcode,index=False)
        groceryList.append(info) 
    groceryList = pd.concat(groceryList,ignore_index=True)
    ...
    # Loop over list of keywords and create/read-in datasets  
    keywordList = [["Liquor Store","Beer","Wine","Bars","Pubs","Cheap Drinks"]]
    beerTobaccoList = [] 
    for keyWord in keywordList: 
        print("Gathering Search Result: " + keyWord[0] + "...")
        csv_zipcode = "data_sets/beerTobacco_" + keyWord[0] + ".csv"
        try:
            info = pd.read_csv(csv_zipcode)
        except: 
            info = ARCGIS.get_data_from_arcGIS_api(uniqueZip,keyWord)
            info['NAME'] = keyWord[0]
            info.to_csv(csv_zipcode,index=False)
        beerTobaccoList.append(info) 
    beerTobaccoList = pd.concat(beerTobaccoList,ignore_index=True)
